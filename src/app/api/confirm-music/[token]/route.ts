...
export async function POST(
  ...
  try {
    ...
    // Find the confirmation with full context
    ...
    if (send?.organization_id && musician && project) {
      try {
        ...
        const subject = allConfirmed
          ? `All musicians confirmed music — ${project.name}`
          : `${musicianName} confirmed music receipt — ${project.name}`

        const html = allConfirmed
          ? `<p><strong>${musicianName}</strong> has confirmed receipt of music for <strong>${project.name}</strong>.</p><p style="color: #16a34a; font-weight: bold;">All ${totalCount} musicians have now confirmed!</p>`
          : `<p><strong>${musicianName}</strong> has confirmed receipt of music for <strong>${project.name}</strong>.</p><p style="color: #6b7280;">${confirmedCount} of ${totalCount} musicians confirmed so far.</p>`
